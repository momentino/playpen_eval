...
class LLMCognitiveFlexibilityTask(Task):
    ...
    def evaluate_wcst(self, model: Model | HF, apply_chat_template:bool, num_evaluations: int, num_trials:int):
        WCST_SYSTEM_PROMPT = """
    ...
    def evaluate_lnt(self, model: Model | HF, apply_chat_template:bool, num_evaluations: int, num_trials:int):
        LNT_SYSTEM_PROMPT = """
    ...
    def evaluate(self, model: Model | HF, apply_chat_template:bool) -> Dict[str, Any]:
        num_evaluations = 8
        num_trials = 25
        wcst_results = self.evaluate_wcst(model,apply_chat_template, num_evaluations, num_trials)
        wcst_acc = sum([r["accuracy"] for r in wcst_results])/len(wcst_results)
        lnt_results = self.evaluate_lnt(model, apply_chat_template, num_evaluations, num_trials)
